...
def calculate_trust_score(trader):
    """
    Returns a dict with the final score AND the individual component
    breakdown, so the API can expose both.
    """
    breakdown = {}
    ...
    consistency_score = min(days_logged / 30, 1.0) * 150
    breakdown['transaction_consistency'] = round(consistency_score, 1)
    score += consistency_score

    # --- 2. Income trend ---
    last_30_sales = recent_transactions.filter(transaction_type='sale').aggregate(
        total=Sum('amount'))['total'] or 0

    sixty_days_ago = date.today() - timedelta(days=60)
    previous_30_sales = trader.transactions.filter(
        transaction_type='sale', date__gte=sixty_days_ago, date__lt=thirty_days_ago
    ).aggregate(total=Sum('amount'))['total'] or 0

    if previous_30_sales > 0:
        growth_ratio = (last_30_sales - previous_30_sales) / previous_30_sales
        trend_score = max(min(growth_ratio, 1.0), -1.0) * 150
    else:
        trend_score = 75 if last_30_sales > 0 else 0
    breakdown['income_trend'] = round(trend_score, 1)
    score += trend_score

    # --- 3. Vouch network strength ---
    vouches_received = trader.vouches_received.select_related('voucher')
    voucher_count = vouches_received.count()
    if voucher_count > 0:
        avg_voucher_score = sum(v.voucher.trust_score for v in vouches_received) / voucher_count
        vouch_score = min(voucher_count / 5, 1.0) * (avg_voucher_score / 850) * 100
    else:
        vouch_score = 0
        avg_voucher_score = 0
    ...
    loan_score = (repaid_count * 20) - (late_count * 10) - (defaulted_count * 80)
